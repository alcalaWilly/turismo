...
class MongoDBService {
  static late Db db;
  static late DbCollection personasCollection;
  static late DbCollection visitasCollection;
  // static late DbCollection categoriasCollection; // Nueva colección

  // Conectar a MongoDB
  static Future<void> connect() async {
    try {
      db = await Db.create(
          "mongodb+srv://travel2024:G1ntWniWxsSNnvAs@cluster0.ipyotgi.mongodb.net/RioTour?retryWrites=true&w=majority&appName=Cluster0");

      await db.open();
      personasCollection = db.collection('usuarios');
      visitasCollection = db.collection('Visitas');
      // categoriasCollection =
      //     db.collection('Categorias'); // Inicializa la colección
  ...
  // static Future<void> insertCategorias(List<Categorias> categorias) async {
  //   try {
  //     for (var categoria in categorias) {
  //       var data = categoria.toMap();
  //       var result = await categoriasCollection.insertOne(data);
  //       if (!result.isSuccess) {
  //         print("Error al insertar la categoría ${categoria.name}");
  //       } else {
  //         print("Categoría ${categoria.name} insertada exitosamente");
  //       }
  //     }
  //   } catch (e) {
  //     print("Error al insertar categorías en MongoDB: $e");
  //   }
  // }

  // Función para obtener categorías desde MongoDB
//   Future<List<Categorias>> fetchCategoriasFromMongoDB() async {
//   final categoriasDocuments =
//       await MongoDBService.categoriasCollection.find().toList();

//   List<Categorias> categoriasList = [];

//   for (var doc in categoriasDocuments) {
//     final nombreCategoria = doc['name'] as String;
//     final iconCategoria = int.parse(doc['icon'].toString());

//     List<Lugar> lugaresList = [];

//     for (var lugarDoc in doc['lugares']) {
//       // Datos básicos del lugar
//       final nombreLugar = lugarDoc['nombre'] as String;
//       final descripcionLugar = lugarDoc['descripcion'] as String;
//       final tiempoEstimado = lugarDoc['tiempoEstimado'] as String;
//       final imagenes = List<String>.from(lugarDoc['imagenes']);

//       // Convertir región a JSON
//       final regionJson = jsonEncode({
//         'southWest': {
//           'lat': lugarDoc['region']['southWest']['lat'],
//           'lng': lugarDoc['region']['southWest']['lng'],
//         },
//         'northEast': {
//           'lat': lugarDoc['region']['northEast']['lat'],
//           'lng': lugarDoc['region']['northEast']['lng'],
//         }
//       });

//       // Convertir polyline a una lista de JSON
//       final polylineJson = (lugarDoc['polyline'] as List)
...
//     // Crear instancia de Categorias y agregar los lugares
//     final categoria = Categorias(
//       name: nombreCategoria,
//       icon: iconCategoria,
//     );
//     categoria.lugares.addAll(lugaresList);
...
// }
  ...
  static Future<Map<String, dynamic>?> insertPersona(
      Map<String, dynamic> data) async {
    try {
      var result = await personasCollection.insertOne(data);
      if (result.isSuccess) {
        return result
            .document; // Retorna el documento insertado, que contiene el '_id'
      }
      return null;
    } catch (e) {
      print("Error al insertar persona en MongoDB: $e");
      return null;
    }
  }

  // Función para insertar una visita en MongoDB y obtener el _id
  static Future<String?> insertVisita(Map<String, dynamic> visitaJson) async {
    try {
      final response = await visitasCollection.insertOne(visitaJson);

      if (response.isSuccess) {
        return response.document!['_id']
            .toString(); // Retorna el _id generado por MongoDB
      } else {
        return null;
      }
    } catch (e) {
      print('Error al insertar visita en MongoDB: $e');
      return null;
    }
  }

  // Función para verificar si una visita ya existe en MongoDB
  static Future<bool> visitaExisteEnMongo(
      Map<String, dynamic> visitaJson) async {
    try {
      final visitaExistente = await visitasCollection.findOne({
        'nombreLugar': visitaJson['nombreLugar'],
        'fechaVisita': visitaJson['fechaVisita']
      });

      return visitaExistente != null;
    } catch (e) {
      print('Error al verificar si la visita existe en MongoDB: $e');
      return false;
    }
  }

  // Función para verificar si una Persona ya existe en MongoDB
  static Future<bool> personaExisteEnMongo(String id) async {
    try {
      final persona =
          await personasCollection.findOne(where.eq('_id', ObjectId.parse(id)));
      return persona != null;
    } catch (e) {
      print("Error al verificar si la persona existe en MongoDB: $e");
      return false;
    }
  }

  // Función para actualizar una Persona
  static Future<void> updatePerson(Map<String, dynamic> data) async {
    try {
      // Asegurarse de que el ObjectId sea válido
      final objectId = ObjectId.tryParse(data['_id']);
      if (objectId == null) {
        print("Error: ObjectId no es válido");
        return;
      }

      // Ejecutar la actualización
      final result = await personasCollection.updateOne(
        where.eq('_id', objectId), // Utilizando el ObjectId correctamente
        modify
            .set('dni', data['dni'])
            .set('pasaporte', data['pasaporte'])
            .set('celular', data['celular'])
            .set('usuario', data['usuario'])
            .set('password', data['password'])
            .set('lugarVista', data['lugarVista'])
            .set('motivos', data['motivos'])
            .set(
                'updatedAt',
                DateTime.parse(data[
                    'updatedAt'])), // Asegurarse de manejar bien las fechas
      );

      // Comprobar el resultado de la actualización
      if (result.isAcknowledged) {
        print("Persona actualizada en MongoDB");
      } else {
        print("Error: No se pudo actualizar la persona en MongoDB");
      }
    } catch (e) {
      print("Error al actualizar persona en MongoDB: $e");
    }
  }

  // Función para obtener visitas sincronizadas desde MongoDB
  // static Future<List<Map<String, dynamic>>>
  //     obtenerVisitasSincronizadas() async {
  //   try {
  //     // Filtrar visitas donde 'syncPending' es false
  //     final visitas =
  //         await visitasCollection.find({'syncPending': false}).toList();
  //     return visitas;
  //   } catch (e) {
  //     print('Error al obtener visitas sincronizadas de MongoDB: $e');
  //     return [];
  //   }
  // }
}

// void syncCategoriasToMongoDB() async {
...
//   await MongoDBService.insertCategorias(categoriaData);
// }
